...
class Haplotype:
    ...
    def get_frequency_info(self) -> HaplotypeFrequency:
        ...
        return HaplotypeFrequency(
            ...
            by_population=self.get_frequency_by_population()
        )
...
def identify_haplotypes_from_alignment(
    ...
    """
    from .alignment import Alignment  # Import here to avoid circular dependency
...
def calculate_haplotype_diversity(haplotypes: List[Haplotype]) -> Dict[str, float]:
    ...
    if total_samples > 1:
        haplotype_diversity = (total_samples / (total_samples - 1)) * (1 - freq_sum_squared)
    else:
        haplotype_diversity = 0.0
