...
class HaplotypeNetwork:
    ...
    def haplotypes(self) -> List[Haplotype]:
        ...
        return [
            hap for hap_id, hap in self._haplotype_map.items()
    ...
    def validate(self) -> None:
        ...
        for source, target, data in self._graph.edges(data=True):
            ...
            if distance < 0:
                raise ValueError(f"Edge ({source}, {target}) has negative distance {distance}")
    ...
    def __str__(self) -> str:
        ...
        return (f"{self.name}: {stats.num_haplotypes} haplotypes, "
