...
class MedianJoiningNetwork(MinimumSpanningNetwork):
    ...
    -------
        ...
        network = self._iterative_median_joining(
            haplotypes, alignment, distance_matrix
        )
    ...
    -------
        ...
        while changed and iteration < max_iterations:
            ...
            msn_length = sum(
                msn.get_edge_distance(u, v) or 0
                for u, v in msn.graph.edges()
            )
            ...
            current_haplotypes = self._remove_obsolete_medians(
                msn, current_haplotypes
            )
            ...
            for median_seq, cost, triplet in candidate_medians:
                if cost <= min_cost + self.epsilon:
                    ...
                    if (self.max_median_vectors and 
                        self._median_counter >= self.max_median_vectors):
                        break
        ...
        final_network = self._build_msn_for_iteration(current_haplotypes, distance_matrix)
    ...
    -------
        ...
        while stack:
            ...
            if first_star == -1:
                ...
                for base in [seq1.data[first_star], seq2.data[first_star], seq3.data[first_star]]:
