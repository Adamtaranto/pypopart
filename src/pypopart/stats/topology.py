...
def identify_ancestral_nodes(
    ...
    for node in G.nodes():
        ...
        max_freq = max(network.get_haplotype(n).frequency 
                      for n in G.nodes() 
    ...
    return {
        ...
        'top_central_nodes': {
            ...
            for method, scores in (
                ('degree', {n: c['degree'] for n, c in centrality.items() if 'degree' in c}),
                ('betweenness', {n: c['betweenness'] for n, c in centrality.items() if 'betweenness' in c})
            )
        }
    }
...
def find_central_hub_nodes(
    ...
    if degree_threshold is None:
        ...
        if len(degree_values) > 1:
            variance = sum((d - mean_degree) ** 2 for d in degree_values) / len(degree_values)
    ...
    # Find hubs
    hubs = [(node, degree) for node, degree in degrees.items() 
