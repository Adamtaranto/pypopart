...
def calculate_haplotype_frequencies(
    ...
    if normalize and total_samples > 0:
        overall_freq = {hap_id: count / total_samples 
                       for hap_id, count in overall_counts.items()}
    else:
    ...
    for pop, counts in pop_counts.items():
        if normalize and pop_totals[pop] > 0:
            by_pop_freq[pop] = {hap_id: count / pop_totals[pop] 
                               for hap_id, count in counts.items()}
        else:
...
def calculate_diversity_metrics(
    ...
    if total_samples <= 1:
        ...
        haplotype_diversity = (total_samples / (total_samples - 1)) * (1 - sum_p_squared)
    ...
    if alignment is not None and total_samples > 1:
        nucleotide_diversity = _calculate_nucleotide_diversity(network, alignment, overall_freq)
...
def calculate_network_metrics(network: HaplotypeNetwork) -> NetworkMetrics:
    ...
    if nx.is_connected(G):
        ...
        avg_path_length = nx.average_shortest_path_length(subgraph) if len(largest_cc) > 1 else 0.0
