...
class TestDetectNetworkPartitions:
    ...
    def test_connected_network(self):
        ...
        haps = [Haplotype(Sequence(f"hap{i}", data), sample_ids=[f"s{i}"])
...
class TestIdentifyAncestralNodes:
    ...
    def test_high_frequency_center(self):
        ...
        center = Haplotype(Sequence("center", "ATCG"), 
                          sample_ids=[f"s{i}" for i in range(10)])
        leaves = [Haplotype(Sequence(f"leaf{i}", data), sample_ids=[f"s{i+10}"])
    ...
    def test_score_components(self):
        ...
        hap1 = Haplotype(Sequence("hap1", "ATCG"), sample_ids=[f"s{i}" for i in range(5)])
