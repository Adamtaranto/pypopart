...
class TestMedianJoiningNetwork:
    ...
    def test_mjn_median_vector_inference(self):
        ...
        median_count = sum(1 for hap_id in [h.id for h in network.haplotypes] 
    ...
    def test_mjn_vs_msn(self):
        ...
        assert len(mjn_network) >= len(msn_network) or len(mjn_network) == len(msn_network)
